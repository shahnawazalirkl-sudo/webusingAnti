...
export function getLastOrder() {
  return safeStorage.getItem<Record<string, unknown>>('asra_last_order', null);
}
...
export function getAllOrders() {
  return safeStorage.getItem<Record<string, Record<string, unknown>>>('asra_orders_history', {}) || {};
}
...
export function formatOrderForTracking(order) {
  ...
  return {
    ...
    phone: phone,
    initials: initials,
    status: 'WHITE-GLOVE SHIPPING QUEUED • DELIVERY PREPARATION',
    ...
    lat: 25.1782,
    lng: 73.8423,
    cruisingSpeed: 'Scheduled for Shipped Segment',
    items: items,
