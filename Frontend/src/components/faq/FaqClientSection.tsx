...
export default function FaqClientSection() {
  ...
  const [ticketStatus, setTicketStatus] = useState<{ state: string; ticketId: string | null; error: string | null }>({
    state: 'idle',
    ticketId: null,
    error: null
  });
  ...
  const handleQuickLookup = (keyword: string) => {
  ...
  const handleVaultLookup = (e: React.FormEvent) => {
    e.preventDefault();
  ...
  return (
    <>
      ...
      <div className="max-w-2xl mx-auto relative shadow-sm rounded-lg bg-white border border-asra-border p-1.5 flex items-center gap-2 focus-within:border-asra-gold focus-within:ring-1 focus-within:ring-asra-gold transition-all">
        ...
        {searchQuery && (
          <button
            ...
            className="text-asra-muted hover:text-asra-dark p-1 rounded-full transition-colors cursor-pointer"
            title="Clear search"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <button
          type="button"
          className="bg-asra-gold hover:bg-asra-goldDark text-white px-5 py-2.5 rounded text-xs font-semibold tracking-wider uppercase transition-colors shrink-0 shadow-xs cursor-pointer"
        >
          Search
        </button>
      </div>
      ...
      <div className="overflow-x-auto pb-2 mt-8 mb-12 scrollbar-none">
            ...
            <div className="bg-white border border-asra-border rounded-xl p-10 text-center space-y-4 shadow-sm">
              ...
              <div className="flex items-center justify-center gap-3 pt-2">
                <button
                  ...
                  className="px-4 py-2 bg-asra-dark text-white rounded text-xs font-semibold uppercase tracking-wider hover:bg-asra-charcoal transition-colors cursor-pointer"
                >
                  Reset Search
                </button>
                <a
                  ...
                  className="px-4 py-2 bg-[#25D366] text-white rounded text-xs font-semibold uppercase tracking-wider hover:bg-[#1EBE5D] transition-colors flex items-center gap-1.5 cursor-pointer"
                >
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-asra-dark text-white rounded-xl p-6 sm:p-7 shadow-lg border border-asra-charcoal relative overflow-hidden">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-full flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1EBE5D] text-white py-3 px-4 rounded-lg font-medium text-xs tracking-wider transition-all shadow-md active:scale-[0.99] uppercase cursor-pointer"
            >
            ...
            {ticketStatus.state === 'success' ? (
              <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-900 space-y-3">
                ...
                <button
                  ...
                  className="text-[11px] underline text-emerald-800 hover:text-emerald-950 font-medium cursor-pointer"
                >
              ...
              <form onSubmit={handleTicketSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full bg-asra-gold hover:bg-asra-goldDark text-white py-2.5 rounded font-medium text-xs tracking-wider uppercase transition-colors shadow-sm cursor-pointer"
                >
          ...
          <div className="bg-asra-cream/50 border border-asra-border rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-2.5 text-xs">
              ...
              <button
                ...
                className="flex flex-col text-left p-3 rounded bg-white border border-asra-border hover:border-asra-gold transition-colors text-asra-dark group shadow-2xs cursor-pointer"
              >
                <span className="text-[10px] text-asra-muted uppercase tracking-wider mb-1 flex items-center gap-1">
      ...
      {/* BOTTOM AUXILIARY PROTOCOL BAR */}
      <aside className="border-t border-asra-border bg-asra-ivory py-4 mt-12 text-xs text-asra-muted -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 sm:gap-6 text-[11px] tracking-wide">
            <button
              ...
              className="hover:text-asra-dark transition-colors underline decoration-asra-border hover:decoration-asra-gold cursor-pointer"
            >
              Support Protocols
            </button>
            <button
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-3.5 py-1.5 rounded border border-asra-border bg-white hover:bg-asra-champagne text-asra-dark text-xs flex items-center gap-1.5 font-medium transition-colors shadow-2xs cursor-pointer"
            >
        ...
        {/* Copyright Subtext Ribbon */}
        <div className="max-w-7xl mx-auto mt-4 pt-3 border-t border-asra-border/40 flex flex-col sm:flex-row justify-between items-center text-[10px] text-asra-muted/80">
          <p>
      ...
      {/* MODALS: PROTOCOLS & BRASS DIE VAULT */}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-asra-border relative animate-fadeIn">
            <button
              ...
              className="absolute top-4 right-4 text-asra-muted hover:text-asra-dark p-1 rounded-full transition-colors cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="mt-6 pt-4 border-t border-asra-border/60 flex justify-end">
              <button
                type="button"
                onClick={() => setActiveProtocol(null)}
                className="px-5 py-2 bg-asra-dark hover:bg-asra-charcoal text-white rounded text-xs font-medium uppercase tracking-wider transition-colors cursor-pointer"
              >
                Close Protocol
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-asra-border relative animate-fadeIn">
            ...
            <form onSubmit={handleVaultLookup} className="flex gap-2 mb-6">
              ...
              <button
                type="submit"
                className="px-4 py-2 bg-asra-gold hover:bg-asra-goldDark text-white text-xs font-semibold uppercase tracking-wider rounded transition-colors cursor-pointer"
              >
                Verify
              </button>
            </form>
            ...
            <div className="mt-6 pt-4 border-t border-asra-border/60 flex items-center justify-between text-[11px] text-asra-muted">
              ...
              <button
                ...
                className="text-asra-dark hover:text-asra-gold font-medium uppercase cursor-pointer"
              >
                Dismiss
              </button>
            </div>
