...
import { PRODUCTS as DEFAULT_PRODUCTS } from '@/data/productsData';
...
export default function ShopFilterBar({
  ...
  const catalog = initialProducts || products || DEFAULT_PRODUCTS;
  ...
  const pillCounts = useMemo(() => {
    ...
    CATEGORY_PILLS.forEach((pill) => {
      if (pill.id === 'all') {
        counts[pill.id] = catalog.length;
      } else if (pill.filterCat) {
        counts[pill.id] = catalog.filter((p) => pill.filterCat.includes(p.category)).length;
      } else if (pill.recipient) {
        counts[pill.id] = catalog.filter((p) => {
