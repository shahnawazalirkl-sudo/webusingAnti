...
export default function TrackOrderClient() {
  ...
  const resolveDocketData = (docketId: string | null) => {
    if (!docketId) return null;
    const cleanId = docketId.trim().toUpperCase();
    if (PRESET_DOCKETS[cleanId as keyof typeof PRESET_DOCKETS]) {
      return PRESET_DOCKETS[cleanId as keyof typeof PRESET_DOCKETS];
    }
    const realOrder = getOrderById(cleanId);
    if (realOrder) {
      return formatOrderForTracking(realOrder);
    }
    return null;
  };
        ...
        <div className="mt-6 bg-white border border-outline-variant/30 rounded-xl p-3 sm:p-5 shadow-sm max-w-3xl mx-auto text-left">
          <form onSubmit={handleLookupSubmit} className="flex flex-col md:flex-row items-center gap-3">
            ...
            <div className="w-full md:w-auto self-end">
              <button
                type="submit"
                className="w-full md:w-auto px-5 py-2.5 bg-primary text-on-primary rounded-lg text-xs font-semibold uppercase tracking-wider shadow-xs hover:bg-[#5f4b2d] active:scale-[0.98] transition-all duration-300 whitespace-nowrap cursor-pointer"
              >
          ...
          <div className="bg-white border border-outline-variant/30 rounded-2xl p-5 sm:p-6 shadow-sm relative overflow-hidden">
            ...
            <div className="space-y-8 mt-6">
              ...
              <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-xl p-4 sm:p-5">
                <div className="flex flex-wrap items-center justify-between gap-2 pb-3 mb-5 border-b border-outline-variant/20">
                  <div className="flex items-center gap-2.5">
                    <span className="w-6 h-6 rounded-full bg-[#1A1A1A] text-white flex items-center justify-center text-[10px] font-bold font-serif">
                      A
                    </span>
                    <div>
              ...
              <div className="bg-white border border-outline-variant/30 rounded-xl p-4 sm:p-5">
                ...
                <ol className="relative pl-6 sm:pl-8 space-y-6 before:content-[''] before:absolute before:left-[11px] sm:before:left-[15px] before:top-2 before:bottom-3 before:w-[1.5px] before:bg-[#E2D6C5]">
                  <li className="relative group bg-[#FAF4EB] -mx-3 p-3.5 rounded-xl border border-[#D8C7B0] shadow-sm">
              ...
              <div className="mt-4 pt-3 border-t border-outline-variant/20 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded bg-surface hover:bg-[#F2ECE3] border border-outline-variant/30 text-on-surface text-[11px] font-semibold transition-colors flex items-center gap-1.5 cursor-pointer"
                  >
          ...
          <div className="bg-white border border-outline-variant/30 rounded-2xl p-5 shadow-sm">
            ...
            <div className="mt-5 pt-4 border-t border-outline-variant/30 grid grid-cols-1 sm:grid-cols-2 gap-2">
              <button
                ...
                className="w-full py-2.5 px-3 rounded-lg border border-outline-variant/30 bg-white hover:bg-surface text-on-surface text-xs font-medium transition-colors flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
              >
              ...
              <button
                ...
                className="w-full py-2.5 px-3 rounded-lg bg-[#1A1A1A] hover:bg-black text-white text-xs font-medium transition-colors flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
              >
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 border border-outline-variant/30 shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button
                type="button"
                onClick={() => setShowChauffeurModal(false)}
                className="py-2.5 px-4 border border-outline-variant/30 hover:bg-surface text-on-surface text-xs font-semibold rounded-xl transition-colors cursor-pointer"
              >
                Dismiss
              </button>
            </div>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 border border-outline-variant/30 shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="mt-5 pt-3 border-t border-outline-variant/30 flex justify-end">
              <button
                type="button"
                onClick={() => setShowHistoryModal(false)}
                className="py-2 px-4 border border-outline-variant/30 hover:bg-surface text-on-surface text-xs font-semibold rounded-lg transition-colors cursor-pointer"
              >
                Close Archive
              </button>
            </div>
