...
export const SUITE_SPECIFICATIONS = [
...
export interface ProductDetailClientProps {
  product: {
    ...
    rating?: number;
    reviewCount?: number;
    editions?: Array<{ name: string; price?: number; dotColors?: string[]; colors?: string[] }>;
    inclusions?: string[];
    [key: string]: any;
  };
}
...
export default function ProductDetailClient({ product }: ProductDetailClientProps) {
  ...
  // Gallery items resolving
  const galleryItems = useMemo(() => {
    if (product.gallery && product.gallery.length > 0) {
      return product.gallery.map((src, idx) => ({
        label: `${product.title} - View ${idx + 1}`,
        src,
        isVideo: false,
      }));
    }
    return SOVEREIGN_ASSETS.gallery;
  }, [product]);

  // Gallery & Presentation State
  const initialHeroImage = product.image || galleryItems[0]?.src || SOVEREIGN_ASSETS.hero;
  const [activeImage, setActiveImage] = useState(initialHeroImage);
  const [activeThumbIndex, setActiveThumbIndex] = useState(0);
  const [isVideoModalOpen, setIsVideoModalOpen] = useState(false);

  // Palette editions resolving
  const paletteEditions = useMemo(() => {
    if (product.editions && product.editions.length > 0) {
      return product.editions.map((ed, idx) => ({
        name: ed.name,
        label: ed.name.split('&')[0]?.trim() || ed.name,
        price: ed.price || product.price,
        colors: ed.dotColors || ed.colors || ['#f4d6d6', '#c5a880'],
        galleryIndex: idx < galleryItems.length ? idx : 0,
      }));
    }
    return PALETTE_EDITIONS;
  }, [product, galleryItems]);

  // Customizer State
  const [selectedEdition, setSelectedEdition] = useState(paletteEditions[0]);
  ...
  // Synchronize state when navigating product
  useEffect(() => {
    window.scrollTo(0, 0);
    setActiveImage(product.image || galleryItems[0]?.src || SOVEREIGN_ASSETS.hero);
    setActiveThumbIndex(0);
    setSelectedEdition(paletteEditions[0]);
  }, [product.id, galleryItems, paletteEditions, product.image]);

  // Handle Thumbnail Selection
  const handleSelectThumbnail = (thumb: { src: string; isVideo?: boolean }, index: number) => {
  ...
  const handleSelectEdition = (edition: typeof paletteEditions[0]) => {
    setSelectedEdition(edition);
    if (galleryItems[edition.galleryIndex]) {
      setActiveImage(galleryItems[edition.galleryIndex].src);
  ...
  const handleAddToCart = () => {
    ...
    toast.success(`Personalized "${product.title}" added to Bag!`, {
      description: `Configured for ${initialsDisplay} • ${selectedEdition.name}`,
    });
  };

  const handleAddCrossSell = (item: { id: string; title: string; price: number; image: string }) => {
    ...
    toast.success(`Added "${item.title}" to your suite!`);
  };
  ...
  const currentPrice = selectedEdition.price || product.price || 7499;
  const originalPrice = product.originalPrice || Math.round(currentPrice * 1.3);
  const totalPrice = currentPrice * orderQty;
            ...
            <div className="relative w-full aspect-[4/5] bg-surface-container-lowest rounded-2xl overflow-hidden shadow-xs group border border-outline-variant/30">
              ...
              <div className="absolute top-3 left-3 sm:top-4 sm:left-4 flex flex-col gap-1.5 sm:gap-2 z-10 pointer-events-none max-w-[calc(100%-75px)]">
                <span className="bg-inverse-surface/90 backdrop-blur-md text-secondary-fixed text-[10px] sm:text-label-sm font-label-sm uppercase tracking-widest px-2.5 sm:px-3 py-1 sm:py-1.5 rounded shadow-sm flex items-center gap-1.5 truncate">
                  ...
                  {product.badge || 'Flagship Masterpiece'}
                </span>
            ...
            <div className="flex flex-col gap-space-xs">
              <div className="flex items-center justify-between">
                <span className="font-label-sm text-label-sm uppercase tracking-[0.2em] text-primary font-semibold">
                  COLLECTIONS • 2026 BRIDAL EDITION • SKU: {product.sku || 'ASRA-SOV-019'}
                </span>
                <button
                  type="button"
              ...
              <div className="flex flex-wrap items-center gap-3 pt-1 pb-2">
                <div className="flex items-center gap-1 text-secondary">
                  ...
                  <span className="font-sans text-xs sm:text-sm font-bold text-on-surface">
                    {product.rating || 4.9}
                  </span>
                  <span className="text-on-surface-variant font-sans text-xs">
                    ({product.reviewCount || 184} Verified Couples &amp; Brides)
                  </span>
                </div>
              ...
              <div className="flex items-baseline gap-3">
                ...
                {discountPercent > 0 && (
                  <span className="px-2.5 py-1 bg-[#FAF4EB] text-primary border border-primary/20 rounded-md text-[10px] font-bold tracking-wider uppercase font-sans">
                    SAVE {discountPercent}% (₹{(originalPrice - currentPrice).toLocaleString('en-IN')} OFF)
                  </span>
                )}
              </div>
              <p className="font-sans text-xs text-on-surface-variant leading-relaxed">
                {product.shortDescription || 'Inclusive of all taxes, complimentary 24k gold leaf calligraphy card, and custom bronze initials wax stamp.'}
              </p>
              <div className="mt-2 pt-2 bg-[#FAF4EB] px-3.5 py-2 rounded-lg flex items-center justify-between border border-primary/20">
                <div className="flex items-center gap-2 text-on-surface">
                  ...
                  <span className="font-sans text-[11px] font-semibold uppercase tracking-wide">
                    Earn {Math.round(currentPrice * 0.1)} Collection Privilege Points + Free Insured Express Delivery
                  </span>
                </div>
              ...
              <div className="flex flex-col gap-space-sm pt-space-xs">
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-space-md">
                  ...
                  <button
                    ...
                    className="flex-1 min-h-[48px] h-12 bg-primary text-on-primary hover:bg-[#5f4b2d] active:scale-[0.98] transition-all duration-300 px-5 rounded-lg shadow-xs flex items-center justify-center gap-2 group touch-manipulation cursor-pointer"
                  >
                    <span className="material-symbols-outlined text-[18px]">
                      shopping_cart
                    </span>
          ...
          <div className="hidden md:flex justify-center mb-8 border-b border-outline-variant/40">
            <div className="inline-flex gap-2 p-1 bg-surface-container rounded-xl">
              <button
                ...
                className={`min-h-[44px] px-6 py-2.5 rounded-lg text-sm font-semibold uppercase tracking-wider transition-all cursor-pointer ${
          ...
          <div className="relative bg-surface-container-lowest max-w-2xl w-full rounded-2xl overflow-hidden shadow-2xl border border-outline-variant/40">
            ...
            <div className="p-4 border-b border-outline-variant/30 flex items-center justify-between bg-surface-container-low">
              ...
              <button
                ...
                className="w-8 h-8 rounded-full flex items-center justify-center text-outline hover:text-on-surface hover:bg-surface-container-high transition-colors cursor-pointer"
              >
            ...
            <div className="relative aspect-video w-full bg-inverse-surface flex items-center justify-center overflow-hidden">
              <img
                src={SOVEREIGN_ASSETS.gallery[5]?.src || SOVEREIGN_ASSETS.hero}
            ...
            <div className="p-4 bg-surface-container-low flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => setIsVideoModalOpen(false)}
                className="px-4 py-1.5 bg-primary text-on-primary text-xs font-semibold uppercase tracking-wider rounded-lg hover:bg-primary/90 transition-colors cursor-pointer"
              >
                Close Preview
              </button>
            </div>
      ...
      <div className="block md:hidden fixed bottom-14 left-0 right-0 z-30 bg-surface/95 backdrop-blur-md px-4 py-2.5 border-t border-border shadow-2xl">
        <div className="flex items-center justify-between gap-3 max-w-lg mx-auto">
          ...
          <button
            type="button"
            onClick={handleAddToCart}
            className="h-11 px-5 bg-primary text-on-primary font-sans text-xs uppercase tracking-wider font-semibold rounded-lg shadow-sm active:scale-95 transition-all flex items-center justify-center gap-1.5 shrink-0 touch-manipulation hover:bg-primary/90 cursor-pointer"
          >
